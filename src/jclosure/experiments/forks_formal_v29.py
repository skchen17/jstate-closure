"""V29 formal all-layer natural token forks and exact native partial write transfer."""
...
SOURCE = "src/jclosure/experiments/forks_formal_v29.py"
...
def native_layers(cache):
    rec = [i for i, layer in enumerate(cache.layers) if isinstance(getattr(layer, "recurrent_states", None), torch.Tensor) and isinstance(getattr(layer, "conv_states", None), torch.Tensor)]
    att = [i for i, layer in enumerate(cache.layers) if isinstance(getattr(layer, "keys", None), torch.Tensor) and isinstance(getattr(layer, "values", None), torch.Tensor)]
    if len(rec) + len(att) != len(cache.layers) or not rec or not att:
        raise RuntimeError("V29 native cache layer coverage incomplete")
    return rec, att
...
def run(root: Path, role: str):
    ...
    verify_stage(root, "interface_amendment")
    if role == "validation":
    ...
    bundle, dense, _, selected_rec, selected_att, _ = context(root)
    scale = scales(root)
    items = d[role]
    if role == "independent_final":
        channels = ("REC+Conv",)
    ...
    for index, item in enumerate(items, 1):
        ...
        if state_hashes(incoming, selected_rec, selected_att) != item["incoming_state_hashes"]:
            raise RuntimeError(f"frozen selected-layer incoming state changed: {item['base_trial_id']}")
        ...
        for condition in channels:
            for direction, recipient, donor, current in (("A_from_B", a0, b0, a1), ("B_from_A", b0, a0, b1)):
                ...
                if condition == "REC+Conv+KV" and (fidelity["state_hashes"] != state_hashes(donor["cache"], rec, att) or metric["relative_l2_to_donor"] > 1e-6):
                    raise RuntimeError(f"full native cache did not reproduce donor: {item['base_trial_id']} {direction}")
    ...
    summary = {"role": role, "states": len(items), "transfer_rows": len(move_rows), "all_writeback_pass": all(x["writeback_pass"] for x in audit_rows), "all_replay_exact": all(x["same_token_replay_exact"] for x in fork_rows), "all_incoming_identical": True, "native_recurrent_layers": rec, "native_attention_layers": att, "native_cache_layer_count": len(rec) + len(att), "future_responses_observed_after_design_freeze": True, "role_response_hashes": {k: sha256_file(root / p) for k, p in paths.items()}, "vectors_sha256": sha256_file(vp), "historical_final_opened": False, "independent_final_opened": role == "independent_final"}
    jp = root / OUT / f"forks_{role}_v29.json"
    write_json_atomic(jp, summary)
    prior = "artifacts/natural_write_content_v29_interface_amendment.freeze.json" if role in ("calibration", "development") else "artifacts/natural_write_content_v29_development_analysis.freeze.json" if role == "validation" else "artifacts/natural_write_content_v29_final_opening.freeze.json"
