"""Frozen bounded two-GPU model-parallel execution for V12."""
...
from jclosure.experiments.protocol_v12_hybrid_nullification import (
    FREEZE_PATH as NULLIFICATION_PATH,
)
...
PROTOCOL_AMENDMENT = "local_causal_geometry_v12_runtime_amendment_5_dual_gpu"
SCHEMA_AMENDMENT = 20
AMENDMENT_PATH = Path(
    "artifacts/causal_geometry_v12_runtime_amendment_5_dual_gpu.freeze.json"
)
CODE_PATH = Path("src/jclosure/experiments/runtime_v12_amendment_5.py")
...
def _load_model_dual_gpu(config: dict[str, Any]) -> Any:
    amended = copy.deepcopy(config)
    amended["model"]["device_map"] = "auto"
    amended["model"]["max_memory"] = {0: "6GiB", 1: "6GiB"}
...
def _freeze() -> None:
    ...
    parents = [BASE_FREEZE_PATH, NULLIFICATION_PATH]
    value: dict[str, Any] = {
        "schema_version": SCHEMA_AMENDMENT,
        "protocol_version": PROTOCOL_AMENDMENT,
        "purpose": "bounded two-GPU model parallelism after finite-value probe",
        ...
        "execution_device": "accelerate auto-map across physical GPUs 0 and 1",
        "per_visible_gpu_memory_limit": "6GiB",
        "cpu_offload": False,
        "probe": {
            "prefill_finite": True,
            "teacher_forced_continuation_finite": True,
            "logits_finite": True,
            "prefill_seconds": 1.83,
...
def _verify(root: Path) -> dict[str, Any]:
    ...
    if value.get("freeze_digest") != _digest(value):
        raise RuntimeError("v12 dual-GPU amendment digest mismatch")
    for name, expected in value["hashes"].items():
        if sha256_file(root / name) != expected:
            raise RuntimeError(f"v12 dual-GPU amendment input changed: {name}")
    return value
...
def main() -> None:
    try:
        ...
        raise SystemExit(
            "usage: runtime_v12_amendment_5 --target freeze|causal|jvp ..."
        ) from exc
    ...
    module.load_model_bundle = _load_model_dual_gpu
