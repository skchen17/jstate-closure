"""Append-only correction of train-calibration zero-vector REC/KV proposals."""
...
SOURCE = "src/jclosure/experiments/q_amendment_v19.py"
REPLACEMENTS = [
    {"name": "rec_arch4_amended", "direction_index": 4, "channel": "recurrent"},
    {"name": "kv_arch14_amended", "direction_index": 14, "channel": "kv"},
]
...
def run(root: Path) -> dict:
    ...
    path = root / bank.OUT / "q_amendment_calibration_v19.parquet"
    frame.to_parquet(path, index=False, compression="zstd")
    corrected = [x for x in initial["q"] if x["name"] not in ("rec_arch9", "kv_finite39")]
    for design in REPLACEMENTS:
        choices = []
        for alpha in cfg["q"]["alpha_candidates"]:
        ...
        corrected.append({**design, **selected, "calibration_status": "RELIABLE" if valid else "NOT_RELIABLY_ACTUATABLE",
                          "natural_active_train_pilot": selected["median_natural_to_action_ratio"] >= cfg["q"]["natural_active_ratio_min"]})
    return stage_freeze(root, "q_amendment_1",
                        [SOURCE, str(path.relative_to(root)), "artifacts/counterfactual_workspace_v19_interventions.freeze.json",
                         "scripts/audit_q_vectors_v19.py"],
                        {"why": "Original REC direction 9 and KV direction 39 were zero in those isolated channels under train-only BF16 calibration; replace with nonzero architecture directions 4 and 14 while preserving the original failed record.",
                         "when": "after 20-state train-only q calibration and before any V19 factorial/validation response",
                         "responses_already_observed": {"q_calibration": 480, "factorial_train": 0, "factorial_validation": 0},
                         "prior_intervention_freeze_digest": initial["freeze_digest"],
