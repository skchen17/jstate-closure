"""Prospectively freeze V37 forks, six probes, triplets and six local sites."""
...
SOURCE = "src/jclosure/experiments/design_v37.py"
...
def run(root: Path, key: str) -> dict:
    ...
    lookup = {row["base_trial_id"]: row["prompt"] for role in ROLES for row in panel[role]}
    ...
    if len(recurrent) != 24 or len(set(recurrent)) != 24:
        raise RuntimeError(f"Recurrent layer topology drift: {key}")
    groups = {name: [recurrent[i] for i in positions]
              for name, positions in cfg["relative_depth_groups"].items()}
    local = {name: recurrent[int(i)] for name, i in cfg["local_relative_positions"].items()}
    if len(local) != 6:
        raise RuntimeError("V37 local position freeze incomplete")
    ...
    for role in ROLES:
        for n, item in enumerate(panel[role], 1):
            ...
            if len(eligible) < 2:
                raise RuntimeError(f"Not enough natural token alternatives: {key}:{sid}")
            ...
            if n % 10 == 0 or n == len(panel[role]):
                print(f"V37 design {key} {role} {n}/{len(panel[role])}", flush=True)
    scales = {}
    for block in BLOCKS:
    ...
    result = {"model_key": key, "model_id": spec["id"], "model_revision": spec["revision"],
              **roles, "target_bundle": bundle, "calibration_clean_scales": scales,
              ...
              "relative_depth_layers": groups, "local_layers": local,
    ...
    path = root / OUT / f"design_{key}_v37.json"
    write_json_atomic(path, result)
    seal = stage_freeze(root, f"design_{key}", [SOURCE, str(path.relative_to(root)),
                                              "artifacts/computational_origin_v37_panel.freeze.json"],
                        {"model_key": key, "design_hash": hd(result),
                         "role_hashes": result["role_hashes"], "local_layer_hash": hd(local),
                         "future_causal_response_observed_before_freeze": False})
...
def finalize(root: Path) -> dict:
    ...
    cfg = verify(root)["config"]
    designs = {key: json.loads((root / OUT / f"design_{key}_v37.json").read_text())
               for key in ("Q", "F")}
    for role in ROLES:
        if [x["base_trial_id"] for x in designs["Q"][role]] != [x["base_trial_id"] for x in designs["F"][role]]:
            raise RuntimeError(f"Cross-model semantic pairing drift: {role}")
    plan = {"roles": {role: [x["base_trial_id"] for x in designs["Q"][role]] for role in ROLES},
            "layers": {key: designs[key]["local_layers"] for key in ("Q", "F")},
    ...
    path = root / OUT / "execution_plan_v37.json"
    write_json_atomic(path, plan)
    seal = stage_freeze(root, "design", [SOURCE, str(path.relative_to(root)),
                                         *[f"artifacts/computational_origin_v37_design_{key}.freeze.json"
                                           for key in ("Q", "F")]],
