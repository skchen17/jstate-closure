"""Response-blind paired V35 forks, endpoints and prospective depth conditions."""
...
SOURCE = "src/jclosure/experiments/design_v35.py"
...
def run(root: Path, key: str):
    ...
    conditions = {name: sorted({layer for group in names for layer in groups[group]})
                  for name, names in cfg["subset_conditions"].items()}
    if len(set(recurrent)) != 24 or conditions["FULL"] != sorted(recurrent):
        raise RuntimeError(f"V35 recurrent mapping drift {key}")
    ...
    for role in ROLES:
        for n, item in enumerate(panel[role], 1):
            ...
            if hashlib.sha256(prompt.encode()).hexdigest() != item["prompt_sha256"]:
                raise RuntimeError(f"V35 prompt drift {sid}")
            incoming, length, ids, logits = prefix(model, tokenizer, key, prompt)
            if length > 512:
                raise RuntimeError(f"V35 context too long {sid}")
            ...
            if anchor not in rank:
                raise RuntimeError(f"V35 anchor ineligible {key}:{sid}")
            eligible = [x for x in library if int(x["token_id"]) in rank and int(x["token_id"]) != anchor]
            if not eligible:
                raise RuntimeError(f"V35 no natural donor token {key}:{sid}")
            ...
            if len(probes) != 6:
                raise RuntimeError("V35 probe shortage")
            ...
            if n % 10 == 0 or n == len(panel[role]):
                print(f"V35 design {key} {role} {n}/{len(panel[role])}", flush=True)
    scales = {}
    ...
    result = {"model_key": key, "model_id": spec["id"], "model_revision": spec["revision"],
              **roles, "anchor_token_id": spec["anchor_token_id"],
              ...
              "condition_layers": conditions,
              "relative_depth_group_hash": hd(groups), "condition_layer_hash": hd(conditions),
...
def finalize(root: Path):
    ...
    q = json.loads((root / OUT / "design_Q_v35.json").read_text())
    f = json.loads((root / OUT / "design_F_v35.json").read_text())
    for role in ROLES:
        if [x["base_trial_id"] for x in q[role]] != [x["base_trial_id"] for x in f[role]]:
            raise RuntimeError("V35 semantic pairing drift")
    plan = {"models": ["Q", "F"], "roles": {role: [x["base_trial_id"] for x in q[role]] for role in ROLES},
            ...
            "conditions_Q": q["condition_layers"], "conditions_F": f["condition_layers"],
            "subset_condition_names": list(cfg["subset_conditions"]),
            "early_cumulative": cfg["early_cumulative"], "late_cumulative": cfg["late_cumulative"],
            "pair_conditions": cfg["pair_conditions"], "leave_out_conditions": cfg["leave_out_conditions"],
            "serial_pairs": cfg["serial_pairs"],
            "gates": {name: value for name, value in cfg.items() if name.endswith("_gate")},
            "final_rule": cfg["final_rule"], "later_horizon_rule": cfg["later_horizon_rule"],
            "thresholds_retuned": False, "causal_response_observed_before_freeze": False}
    path = root / OUT / "execution_plan_v35.json"
    write_json_atomic(path, plan)
    seal = stage_freeze(root, "design", [SOURCE, str(path.relative_to(root)),
                                         "artifacts/hierarchical_read_v35_design_Q.freeze.json",
                                         "artifacts/hierarchical_read_v35_design_F.freeze.json"],
    ...
    return {"freeze_digest": seal["freeze_digest"], "paired_states": sum(len(plan["roles"][r]) for r in ROLES),
            "depth_conditions": len(plan["subset_condition_names"])}
