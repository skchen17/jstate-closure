"""Fresh paired semantic V36 states disjoint from V28–V35 formal panels."""
...
SOURCE = "src/jclosure/experiments/panel_v36.py"
...
def run(root: Path):
    ...
    previous = json.loads((root / "results/v35/processed/panel_v35.json").read_text())
    used.update(row["base_trial_id"] for role in ROLES for row in previous[role])
        ...
        for source in ("train", "validation"):
            ...
            for role, count in assignment:
                if len(candidates) < offset + count:
                    raise RuntimeError(f"V36 insufficient disjoint states {family}/{source}")
    ...
    if len(ids) != 180 or len(set(ids)) != 180 or set(ids) & used:
        raise RuntimeError("V36 panel disjointness violation")
