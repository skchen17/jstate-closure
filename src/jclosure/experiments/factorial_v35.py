"""Fresh V35 native REC×Conv factorial before any hierarchical depth inference."""
...
SOURCE = "src/jclosure/experiments/factorial_v35.py"
...
def run(root: Path, key: str, role: str):
    ...
    if role == "independent_final":
        ...
        if not opening["opened"]:
            raise RuntimeError("V35 independent final sealed")
    ...
    for n, item in enumerate(design[role], 1):
        ...
        if hd(ids) != item["prefix_token_hash"] or field_hashes(incoming) != item["incoming_state_hashes"]:
            raise RuntimeError(f"V35 incoming drift {key}:{sid}")
        ...
        for condition, channels in (("Y10", ["REC"]), ("Y01", ["Conv"]), ("Y11", ["REC", "Conv"])):
            ...
            if field_hashes(caches[condition])["KV"] != recipient_hashes["KV"]:
                raise RuntimeError("V35 primary KV changed")
        responses = {}
        ...
        if n % 10 == 0 or n == len(design[role]):
            print(f"V35 factorial {key} {role} {n}/{len(design[role])}", flush=True)
    frame = pd.DataFrame(rows)
    files = {"rows": root / OUT / f"factorial_{key}_{role}_v35.parquet",
             "vectors": root / OUT / f"factorial_vectors_{key}_{role}_v35.npz",
             "audit": root / OUT / f"factorial_audit_{key}_{role}_v35.parquet"}
    ...
    path = root / OUT / f"factorial_{key}_{role}_v35.json"
