"""Assemble presealed, historically disjoint V37 task pools into one panel."""
...
SOURCE = "src/jclosure/experiments/panel_v37.py"
...
def run(root: Path) -> dict:
    ...
    for role in ROLES:
        ...
        if len(rows) != expected:
            raise RuntimeError(f"V37 {role} pool count mismatch")
        for family in cfg["families"]:
            if sum(row["family"] == family for row in rows) != cfg["roles_per_family"][role]:
                raise RuntimeError(f"V37 {role}/{family} count mismatch")
        for row in rows:
            prompt_hash = hashlib.sha256(row["prompt"].encode()).hexdigest()
            if row["prompt_sha256"] != prompt_hash or row["role"] != role:
                raise RuntimeError(f"V37 {role} prompt/role drift")
            for value, bucket in ((row["base_trial_id"], ids),
                                  (row["program_hash"], programs),
                                  (prompt_hash, prompts)):
                if value in bucket:
                    raise RuntimeError(f"V37 role-pool overlap: {role}:{value}")
                bucket.add(value)
        roles[role] = rows
    if len(ids) != 180:
        raise RuntimeError("V37 panel must contain 180 distinct states")
    result = {**roles, "source": "new_v37_generated_task_pools",
              "v18_train_states_in_formal_roles": 0,
              "historical_v28_v36_states_in_formal_roles": 0,
    ...
    path = root / OUT / "panel_v37.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    write_json_atomic(path, result)
    seal = stage_freeze(root, "panel", [SOURCE, str(path.relative_to(root)),
                                        "artifacts/computational_origin_v37_sample_pools.freeze.json"],
