"""Freeze or execute the final protocol-v9 reporting amendment."""
...
from jclosure.protocol_v9_reporting_amendment_2 import (
    build_amendment,
    verify_amendment_2,
)
from jclosure.reporting_v9_amendment_2 import build_reports


def main() -> None:
    parser = standard_parser(
        "protocol-v9 reporting amendment 2",
        "configs/sufficiency_v9.yaml",
    )
    parser.add_argument("--stage", required=True, choices=("freeze", "report"))
    args = parser.parse_args()
    context = initialize_context("report-v9-amendment-2", args)
    try:
        if args.stage == "freeze":
            amendment = build_amendment(context.root, context.config)
            context.finish("COMPLETED_V9_REPORTING_AMENDMENT_2_FREEZE", **amendment)
            return
        ...
        outputs = build_reports(
            context.root, context.config, base, amendment_1, amendment_2
        )
        context.finish(
            "COMPLETED_V9_REPORTING_AMENDMENT_2",
            base_freeze_digest=base["freeze_digest"],
            amendment_digest=amendment_2["amendment_digest"],
            reports=outputs,
        )
