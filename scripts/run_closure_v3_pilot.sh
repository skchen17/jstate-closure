...
CONFIG="${CONFIG:-configs/closure_v3_pilot.yaml}"
SHARD_GROUP_ID="${JCLOSURE_SHARD_GROUP_ID:-closure-v3-pilot-$(date -u +%Y%m%dT%H%M%SZ)}"
...
timeout 24h python -m jclosure.experiments.closure_v3 \
  ...
  --run-suffix closure-pilot-shard-000 "$@" &
PID0=$!
timeout 24h python -m jclosure.experiments.closure_v3 \
  --config "$CONFIG" --device 1 --shard-index 1 --shard-count 2 \
  --shard-group-id "$SHARD_GROUP_ID" \
  --run-suffix closure-pilot-shard-001 "$@" &
PID1=$!
...
if (( STATUS0 != 0 || STATUS1 != 0 )); then
  echo "Closure pilot shards failed: shard-000=$STATUS0 shard-001=$STATUS1" >&2
  exit 1
fi
