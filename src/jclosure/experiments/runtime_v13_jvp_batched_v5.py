"""V13 batched exact-JVP amendment 5: isolated spectrum diagnostic."""
...
AMENDMENT_PATH = Path("artifacts/causal_geometry_v13_jvp_batched_v5.freeze.json")
CODE_PATH = Path("src/jclosure/experiments/runtime_v13_jvp_batched_v5.py")
...
PROTOCOL = "causal_path_geometry_v13_jvp_batched_amendment_5"
DIAGNOSTIC_ROOT = Path("results/v13/processed/jvp_matrices_batched_diagnostic")
...
def _freeze(root: Path) -> dict[str, Any]:
    ...
    parent = json.loads((root / v4.AMENDMENT_PATH).read_text())
    value: dict[str, Any] = {
        "schema_version": 22,
        "protocol_version": PROTOCOL,
        "purpose": "write an isolated batched matrix for scalar-versus-batched spectrum adjudication",
        ...
        "spectrum_acceptance_gate": {
            "prefix_r90_r95_r99_exact_match": True,
            "maximum_mean_absolute_difference": 0.001,
            "minimum_mean_column_cosine": 0.999,
            "minimum_worst_column_cosine": 0.998,
            "maximum_relative_frobenius_error": 0.02,
        },
