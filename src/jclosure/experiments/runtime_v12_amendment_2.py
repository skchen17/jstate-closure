"""Frozen CPU-execution fallback for V12 causal sweeps on a shared GPU host."""
...
from jclosure.experiments.runtime_v12_amendment import (
    AMENDMENT_PATH as AMENDMENT_1_PATH,
)
...
PROTOCOL_AMENDMENT = "local_causal_geometry_v12_runtime_amendment_2_cpu"
SCHEMA_AMENDMENT = 17
AMENDMENT_PATH = Path(
    "artifacts/causal_geometry_v12_runtime_amendment_2_cpu.freeze.json"
)
CODE_PATH = Path("src/jclosure/experiments/runtime_v12_amendment_2.py")
...
def _load_model_cpu(config: dict[str, Any]) -> Any:
    amended = copy.deepcopy(config)
    amended["model"]["device_map"] = "cpu"
...
def _freeze() -> None:
    ...
    parents = [BASE_FREEZE_PATH, AMENDMENT_1_PATH]
    value: dict[str, Any] = {
        "schema_version": SCHEMA_AMENDMENT,
        "protocol_version": PROTOCOL_AMENDMENT,
        "purpose": "CPU execution fallback while both shared GPUs lack safe capacity",
        ...
        "reason": (
            "both physical GPUs were occupied by unrelated jobs; two pre-record OOM "
            "attempts occurred before any V12 measurement row was emitted"
        ),
        "hashes": {
...
def _verify(root: Path) -> dict[str, Any]:
    ...
    if value.get("freeze_digest") != _digest(value):
        raise RuntimeError("v12 CPU amendment digest mismatch")
    for name, expected in value["hashes"].items():
        if sha256_file(root / name) != expected:
            raise RuntimeError(f"v12 CPU amendment input changed: {name}")
    return value
...
def main() -> None:
    try:
        ...
        raise SystemExit(
            "usage: runtime_v12_amendment_2 --target freeze|causal|jvp ..."
        ) from exc
    ...
    module.load_model_bundle = _load_model_cpu
