"""Generate and seal independent, historical-disjoint V37 task pools.

This stage performs no model inference or causal intervention. Role-specific
seeds and immutable output hashes precede all V37 response observation.
"""
...
from jclosure.protocol_v37 import stage_freeze, verify
from jclosure.provenance import sha256_file, write_json_atomic


SOURCE = "src/jclosure/experiments/sample_pool_v37.py"
OUT = Path("data/v37")
...
def run(root: Path) -> dict:
    ...
    if OUT.exists() and any(OUT.iterdir()):
        raise RuntimeError("V37 sample pool already exists; never regenerate it in place")
        ...
        for fi, family in enumerate(cfg["families"]):
            ...
            for task in tasks:
                ...
                if prompt_hash in blocked_prompts:
                    raise RuntimeError(f"V37 prompt collision in {role}/{family}: {task.example_id}")
                blocked_programs.add(task.program_hash)
                blocked_prompts.add(prompt_hash)
                row = {"base_trial_id": task.example_id.replace("v8-", f"v37-{role}-", 1),
                ...
                rows.append(row)
                all_rows.append(row)
        if len(rows) != sum(cfg["roles_per_family"][role] for _ in cfg["families"]):
            raise RuntimeError(f"V37 {role} count mismatch")
        if len({row["base_trial_id"] for row in rows}) != len(rows):
            raise RuntimeError(f"V37 {role} repeated IDs")
        path = root / OUT / f"{role}_pool_v37.json"
    ...
    if len(all_rows) != 180 or len({row["program_hash"] for row in all_rows}) != 180:
        raise RuntimeError("V37 role pools overlap")
    manifest = {"protocol": "v37", "roles": role_summaries, "historical_sources": historical_sources,
                "historical_program_overlap": False, "historical_prompt_overlap": False,
                ...
                "historical_v36_results_used_in_selection": False}
    path = root / OUT / "sample_pool_manifest_v37.json"
    write_json_atomic(path, manifest)
    stage_freeze(root, "sample_pools", [SOURCE, str(path.relative_to(root)),
                                        *[str((OUT / f"{role}_pool_v37.json")) for role in ROLES],
                                        *[f"artifacts/computational_origin_v37_pool_{role}.freeze.json"
                                          for role in ROLES]],
